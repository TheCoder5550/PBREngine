export function CreateHDR(data, width = 1024, height = 512, name = "new") {
...
export async function LoadHDR(path) {
  ...
  var pos = 0;
  var d8 = new Uint8Array(arrayBuffer);
  ...
  for (let j = 0; j < height; j++) {
    const rgbe = d8.subarray(pos, pos += 4);
    // let scanline = [];
    // let scanline = new Array(width * 4);
    // const scanline = new Uint8Array(width * 4);
    ...
    if (rgbe[0] != 2 || (rgbe[1] != 2) || (rgbe[2] & 0x80)) {
      ...
      while (len > 0) {
        ...
        if (img[ipos] == 1 && img[ipos + 1] == 1 && img[ipos + 2] == 1) {
          for (img[ipos + 3] << rs; i > 0; i--) { // bruh, where does 'i' come from?
      ...
      for (var i = 0; i < 4; i++) {
        ...
        while (ptr < ptr_end) {
          ...
          if (buf0 > 128) {
            ...
            // const l = count;
            // const sub = d8.subarray(pos, pos + l);
            // scanline.set(sub, ptr);
            // ptr += l;
            // pos += l;
            // count = 0;
...
// export async function LoadHDR(path, exposure = 1, gamma = 1) {
