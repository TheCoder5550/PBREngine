...
async function setup() {
  ...
  var solidColorInstanceProgram = new renderer.ProgramContainer(await renderer.createProgramFromFile(renderer.path + "assets/shaders/custom/webgl2/solidColor"));
  ...
  scene.add(await renderer.loadGLTF(renderer.path + "assets/models/porsche-911-carrera-4s.glb"));
  // var helmet = scene.add(await renderer.loadGLTF(renderer.path + "assets/models/DamagedHelmetTangents.glb"));

  physicsEngine.fixedUpdate = function(dt) {}

  var albedo = await renderer.loadTextureAsync(renderer.path + "assets/textures/Snow/albedo.jpg");
  var normal = await renderer.loadTextureAsync(renderer.path + "assets/textures/Snow/normal.jpg");
